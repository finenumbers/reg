...
5. Only then enable Settings **регулярная загрузка** (`regsPollEnabled`) with a sane interval (still one replica). This also schedules phones and groups (one `export.py` at a time).
