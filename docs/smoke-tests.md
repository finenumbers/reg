...
10. **Statistics** — `/stats` (admin and operator, `phones:read`) month switcher; table order ТфОП, внешняя нумерация (`Trunk_`), platforms. ТфОП is one row per join name (`PSTN_*` without `_Local`/`_LDC`): Local groups from `*_Local`/unsuffixed, **Межгород** = outgoing only from the paired `*_LDC`. ТфОП and внешняя нумерация show incoming parking (`Service_Parking`) and phantom (parking + both sides «Нет в биллинге»). Empty cells show «-». A call that matches several categories is counted in each.
