...
9. **Storage (admin)** — `/settings` (bottom, after API keys) shows the CDR month table (calls / seconds / minutes / delete) with no outer card (same chrome as `/traffic`). Minutes are the sum of per-call ceiled minutes, not total seconds / 60. Delete is only on the oldest complete month; confirm by typing `YYYY-MM`. No «Хранение данных» nav item. `/storage` redirects to `/settings#cdr-storage` for admin; operator opening `/storage` or `/settings` → `/forbidden`.
