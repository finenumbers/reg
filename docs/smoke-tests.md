...
10. **Statistics** — `/stats` (admin and operator, `phones:read`) month switcher; table order ТфОП, внешняя нумерация (`Trunk_`), platforms. No outer card on any table. ТфОП and внешняя нумерация share a two-level header (Входящий/Исходящий трафик, Входящий паркинг, Фантомный трафик × Звонки/Минуты); **Межгород** only on ТфОП. ТфОП is one row per join name (`PSTN_*` without `_Local`/`_LDC`): Local groups from `*_Local`/unsuffixed, **Межгород** = outgoing only from the paired `*_LDC`. Minutes are bold; «Итого» minute cells are yellow. Empty cells show «-». A call that matches several categories is counted in each.
