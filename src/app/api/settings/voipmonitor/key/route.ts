...
/**
 * PUT /api/settings/voipmonitor/key — replace VoIPmonitor API password.
 */
export async function PUT(request: Request) {
  ...
  try {
    const parsed = voipmonitorPasswordReplaceSchema.parse(body);
    const ip = await getRequestIp();
    const settings = await replaceVoipmonitorPassword(parsed, { userId, ip });
