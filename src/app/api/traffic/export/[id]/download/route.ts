...
import { getMonthExportForOwner } from "@/modules/traffic/month-export-job";
import { monthExportOutputPath } from "@/modules/traffic/month-export-paths";

export const runtime = "nodejs";

/**
 * GET /api/traffic/export/:id/download — stream completed XLSX.
 */
export async function GET(
  ...
  const filePath = monthExportOutputPath(id);
  ...
  return new NextResponse(webStream, {
    ...
    headers: {
      ...
      "Content-Disposition": xlsxContentDisposition(
        job.filename || "traffic.xlsx",
      ),
